...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!cell) {
        if (_images.count > 0) {
            ...
            cell.lineView.frame = CGRectMake(0, _itemHeight, _itemWidth, 1);
        }
        cell.textLabel.numberOfLines = 0;
    }

    if (indexPath.row == _indexSelectedCell) {
        cell.backgroundColor = KBigLine_Color;
        cell.textLabel.textColor = kBlue100;
    } else {
...
- (void)setDefaultSettings
{
    ...
    _menuBackView.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:KUser.isNightType ? 0.5 : 0.2];
    _menuBackView.alpha = 0;
...
- (void)setIndexSelectedCell:(NSInteger)indexSelectedCell {
    _indexSelectedCell = indexSelectedCell;
    [self.tableView reloadData];
}
...
- (void)updateUI
{
    ...
    if (_arrowDirection == YBPopupMenuArrowDirectionTop) {
        ...
        if (_arrowPosition > _itemWidth / 2) {
            self.frame = CGRectMake(YBScreenWidth - _minSpace - _itemWidth, y - Kscal(70), _itemWidth, height + _arrowHeight);
        }else if (_arrowPosition < _itemWidth / 2) {
            self.frame = CGRectMake(_minSpace, y - Kscal(70), _itemWidth, height + _arrowHeight);
        }else {
            self.frame = CGRectMake(_point.x - _itemWidth / 2, y - Kscal(70), _itemWidth, height + _arrowHeight);
        }
