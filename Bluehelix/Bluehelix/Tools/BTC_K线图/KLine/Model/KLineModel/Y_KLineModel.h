...
/** 数据类型 */
@property (strong) NSString *kLineType;

/**
 *  该Model及其之前所有收盘价之和
 */
@property (strong) NSNumber *SumOfLastClose;

/**
 *  该Model及其之前所有成交量之和
 */
@property (strong) NSNumber *SumOfLastVolume;
 ...
 *  开盘价
 */
@property (strong) NSNumber *Open;


/**
 *  收盘价
 */
@property (strong) NSNumber *Close;
...
//MA（5）=（C1+C2+……CN）/5
@property (strong) NSNumber *MA5;

//MA（10）=（C1+C2+……CN）/10
@property (strong) NSNumber *MA10;

//MA（30）=（C1+C2+……CN）/30
@property (strong) NSNumber *MA30;

@property (strong) NSNumber *Volume_MA5;

@property (strong) NSNumber *Volume_MA10;

@property (strong) NSNumber *Volume_EMA5;

@property (strong) NSNumber *Volume_EMA10;

#pragma BOLL线

@property (strong) NSNumber *MA20;

// 标准差 二次方根【 下的 (n-1)天的 C-MA二次方 和】
@property (strong) NSNumber *BOLL_MD;

// n-1 天的 MA
@property (strong) NSNumber *BOLL_MB;

// MB + k * MD
@property (strong) NSNumber *BOLL_UP;

// MB - k * MD
@property (strong) NSNumber *BOLL_DN;

//  n 个 ( Cn - MA20)的平方和
@property (strong) NSNumber *BOLL_SUBMD_SUM;

// 当前的 ( Cn - MA20)的平方
@property (strong) NSNumber *BOLL_SUBMD;


#pragma 第一个EMA等于MA；即EMA(n) = MA(n)
// EMA（N）=2/（N+1）*（C-昨日EMA）+昨日EMA；
@property (strong) NSNumber *EMA5;

// EMA（N）=2/（N+1）*（C-昨日EMA）+昨日EMA；
@property (strong) NSNumber *EMA10;

// EMA（N）=2/（N+1）*（C-昨日EMA）+昨日EMA；
@property (strong) NSNumber *EMA30;

// EMA（N）=2/（N+1）*（C-昨日EMA）+昨日EMA；
@property (strong) NSNumber *EMA12;

// EMA（N）=2/（N+1）*（C-昨日EMA）+昨日EMA；
@property (strong) NSNumber *EMA26;
...
@property (strong) NSNumber *DIF;

//今日的DEA值（即MACD值）=前一日DEA*8/10+今日DIF*2/10.
@property (strong) NSNumber *DEA;

//EMA（12）=昨日EMA（12）*11/13+C*2/13；   即为MACD指标中的快线；
//EMA（26）=昨日EMA（26）*25/27+C*2/27；   即为MACD指标中的慢线；
@property (strong) NSNumber *MACD;

/**
 *  9Clock内最低价
 */
@property (strong) NSNumber *NineClocksMinPrice;


/**
 *  9Clock内最高价
 */
@property (strong) NSNumber *NineClocksMaxPrice;
...
//D(3日)=（当日K值+2*前一日D值）÷3
//J=3K－2D
@property (strong) NSNumber *RSV;

@property (strong) NSNumber *KDJ_K;

@property (strong) NSNumber *KDJ_D;

@property (strong) NSNumber *KDJ_J;
