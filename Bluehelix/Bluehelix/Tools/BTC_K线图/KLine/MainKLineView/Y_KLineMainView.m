...
- (void)drawRect:(CGRect)rect{
    ...
    if(self.MainViewType == Y_StockChartcenterViewTypeKline || self.MainViewType ==  Y_StockChartcenterViewTypeTimeLine)
    {
        ...
        if ( self.needDrawKLineModels.lastObject == self.kLineModels.lastObject ) {
            NSString *closeString = [KDecimal decimalNumber:[NSString stringWithFormat:@" %.12f", [lastModel.Close doubleValue]] RoundingMode:NSRoundDown scale:KDetail.priceDigit];
...
- (NSInteger)getNeedDrawStartIndexWithScroll:(BOOL)scorll
{
    if (scorll)
    {
        CGFloat klineWidth = [Y_StockChartGlobalVariable kLineGap] + [Y_StockChartGlobalVariable kLineWidth];
        double scrollViewOffsetX = ((NSInteger)(self.parentScrollView.contentOffset.x / klineWidth))*klineWidth;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    if([keyPath isEqualToString:Y_StockChartContentOffsetKey])
    {
        ...
        if (self.parentScrollView.contentOffset.x < -10 && KDetail.isLoading == NO && KDetail.isNext == YES && KDetail.blockLoadNext) {
        ...
        {
            CGFloat klineWidth = [Y_StockChartGlobalVariable kLineGap] + [Y_StockChartGlobalVariable kLineWidth];
            self.oldContentOffsetX = ((NSInteger)(self.parentScrollView.contentOffset.x / klineWidth))*klineWidth;
            if (self.oldContentOffsetX < 0) {
                self.oldContentOffsetX = 0;
            }
            [self drawMainView];
            [self mas_updateConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self.parentScrollView).offset(self.oldContentOffsetX);
