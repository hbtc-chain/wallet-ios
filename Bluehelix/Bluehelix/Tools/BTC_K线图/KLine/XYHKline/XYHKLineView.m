...
- (void)setKLineModels:(NSArray *)kLineModels
{
    ...
    [self reloadMainAccessoryTopData];
...
- (void)insertLineData:(id)data insertCount:(NSInteger)insertCount {
    ...
    double lineGap = [Y_StockChartGlobalVariable kLineGap];
    double lineWidth = [Y_StockChartGlobalVariable kLineWidth];
    double offset = insertCount*(lineWidth + lineGap);
    if (offset > 0)
    {
        self.scrollView.contentOffset = CGPointMake(offset, 0);
    } else {
...
- (void)updateLineData:(NSArray *)data {
    ...
    if (KDetail.isReloadKlineUI) {
        ...
        [self.kLineMainView updateLineData:_kLineModels];
    }
...
- (void)reloadMainAccessoryTopData {
    if (_kLineModels.count > 0) {
        if (self.tickerWindow.hidden == YES) {
