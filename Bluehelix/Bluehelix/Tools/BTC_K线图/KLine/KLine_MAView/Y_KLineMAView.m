...
-(void)maProfileWithModel:(Y_KLineModel *)model
{
    ...
    if (self.MainViewType == Y_StockChartcenterViewTypeTimeLine || [Y_StockChartGlobalVariable mainViewLineStatus] == Y_StockChartTargetLineStatusCloseMA) {
        ...
        if ([model.EMA5 doubleValue] == 0) {
            _MA5Label.text = @"";
        } else {
            _MA5Label.text = [NSString stringWithFormat:@" EMA5:%@ ", [KDecimal decimalNumber:[NSString stringWithFormat:@"%.12f", [model.EMA5 doubleValue]] RoundingMode:NSRoundDown scale:KDetail.priceDigit]];
        }
        
        if ([model.EMA10 doubleValue] == 0) {
            _MA10Label.text = @"";
        } else {
            _MA10Label.text = [NSString stringWithFormat:@" EMA10:%@ ", [KDecimal decimalNumber:[NSString stringWithFormat:@"%.12f", [model.EMA10 doubleValue]] RoundingMode:NSRoundDown scale:KDetail.priceDigit]];
        }

        if ([model.EMA30 doubleValue] == 0) {
            _MA30Label.text = @"";
        } else {
            _MA30Label.text = [NSString stringWithFormat:@" EMA30:%@",[KDecimal decimalNumber:[NSString stringWithFormat:@"%.12f", [model.EMA30 doubleValue]] RoundingMode:NSRoundDown scale:KDetail.priceDigit]];
        }
    }
