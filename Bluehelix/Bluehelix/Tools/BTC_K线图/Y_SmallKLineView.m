...
- (XYHKLineView *)kLineView
{
    if(!_kLineView)
    {
        ...
        _kLineView.targetLineStatus = [KDetail.klineMainIndex integerValue];
    }
    return _kLineView;
}
...
- (void)clickSegmentButtonIndex:(NSInteger)index
{
    ...
    if(index >= 100) {
        ...
        [self.kLineView reloadMainAccessoryTopData];
    } else {
...
- (void)insertLineData:(id)data insertCount:(NSInteger)insertCount {
    [self.kLineView insertLineData:data insertCount:insertCount];
}
