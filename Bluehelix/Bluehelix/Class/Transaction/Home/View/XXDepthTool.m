...
+ (NSDictionary *)depthLeftDict:(NSMutableDictionary *)leftDict
    ...
    for (NSInteger i=0; i < 10; i ++) {
        if (i < leftKeysArray.count) {
            ...
            if ([model.number doubleValue] > maxAmount) {
                maxAmount = [model.number doubleValue];
            }
        }
    ...
    BOOL isSuccess = YES;
    if (rightKeysArray.count > 0 && leftKeysArray.count > 0 && [leftKeysArray.firstObject doubleValue] > [rightKeysArray.firstObject doubleValue]) {
        isSuccess = NO;
    }
    return @{
        @"success":@(isSuccess),
...
+ (NSDictionary *)klineDepthLeftDict:(NSMutableDictionary *)leftDict
    ...
    leftMidPrice = [[NSString stringWithFormat:@"%.13f", leftMidPrice] doubleValue];
    rightMaxPrice = [[NSString stringWithFormat:@"%.13f", rightMaxPrice] doubleValue];
    ...
    if (leftMaxPrice == 0) {
        centerPrice = rightMidPrice;
    }
    if (rightMidPrice == 0) {
        centerPrice = leftMaxPrice;
    }
    
    CGFloat yinzi = 0.01;
    double leftCenter = centerPrice;
    if ((centerPrice - leftMaxPrice)/(centerPrice - leftMidPrice) > yinzi) {
        leftCenter = leftMaxPrice + (leftMaxPrice - leftMidPrice)*yinzi;
    }
    CGFloat lxScale = viewWidth*0.5 / (leftCenter - leftMidPrice);
    
    double rightCenter = centerPrice;
    if ((rightMidPrice - centerPrice)/(rightMaxPrice - centerPrice) > yinzi) {
        rightCenter = rightMidPrice - (rightMaxPrice - rightMidPrice)*yinzi;
    }
    CGFloat rxScale = viewWidth*0.5 / (rightMaxPrice - rightCenter);
    ...
    for (NSInteger i=0; i < leftModelsArray.count; i ++) {
        ...
        if (i==0) {
            ...
            if (leftModelsArray.count > 1) {
                ...
                CGFloat x = (model.price - leftMidPrice)*lxScale;
                CGFloat nextX = (nextModel.price - leftMidPrice)*lxScale;
              ...
              model.endPoint = CGPointMake((model.price - leftMidPrice)*lxScale, model.startPoint.y);
              [leftPath addLineToPoint:model.endPoint];
              [leftFullPath addLineToPoint:model.endPoint];
      ...
      for (NSInteger i=0; i < rightModelsArray.count; i ++) {
          ...
          if (i==0) {
              ...
              model.startPoint = CGPointMake((model.price - rightCenter)*rxScale + viewWidth*0.5, viewHeight - model.orderNumber*yScale);
              ...
              if (rightModelsArray.count > 1) {
                  ...
                  CGFloat x = (model.price - rightCenter)*rxScale + viewWidth*0.5;
                  CGFloat nextX = (nextModel.price - rightCenter)*rxScale + viewWidth*0.5;
              ...
              model.endPoint = CGPointMake((model.price - rightCenter)*rxScale + viewWidth*0.5, model.startPoint.y);
    ...
    for (NSInteger i=0; i < 20; i ++) {
        ...
        // 左侧数据
        NSInteger j = leftKeysArray.count - 1 - i;
        if (j >= 0 ) {
            NSString *leftKey = leftKeysArray[j];
            orderModel.leftPrice = [KDecimal decimalNumber:leftKey RoundingMode:NSRoundDown scale:KDetail.priceDigit];
            orderModel.leftNumber = [KDecimal decimalNumber:leftDict[leftKey] RoundingMode:NSRoundDown scale:KDetail.numberDigit];
            leftSum += [orderModel.leftNumber doubleValue];
            orderModel.leftSumNumber = leftSum;
            
            if ([orderModel.leftNumber doubleValue] > maxAmount) {
                maxAmount = [orderModel.leftNumber doubleValue];
            }
        } else {
        ...
        // 右侧数据
        if (i < rightKeysArray.count) {
            NSString *rightKey = rightKeysArray[i];
            orderModel.rightPrice = [KDecimal decimalNumber:rightKey RoundingMode:NSRoundDown scale:KDetail.priceDigit];
            orderModel.rightNumber = [KDecimal decimalNumber:rightDict[rightKey] RoundingMode:NSRoundDown scale:KDetail.numberDigit];
            rightSum += [orderModel.rightNumber doubleValue];
            orderModel.rightSumNumber = rightSum;
            
            if ([orderModel.rightNumber doubleValue] > maxAmount) {
                maxAmount = [orderModel.rightNumber doubleValue];
            }
        } else {
    ...
    BOOL isSuccess = YES;
    if (leftMaxPrice > rightMidPrice && rightKeysArray.count > 0) {
        isSuccess = NO;
    }
