...
- (void)tapAction {
    UIWindow* window = [[UIWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
       ...
       for (NSInteger i=0; i < KUser.accounts.count; i ++) {
           ...
           NSDictionary *dic = KUser.accounts[i];
           
           UIAlertAction *action = [UIAlertAction actionWithTitle:dic[@"BHAddress"] style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
               KUser.rootAccount = dic;
               
               self.addressLabel.text = KUser.rootAccount[@"BHAddress"];
...
- (XXLabel *)addressLabel {
    if (!_addressLabel) {
        CGFloat width = [NSString widthWithText:KUser.rootAccount[@"BHAddress"] font:kFont(13)];
        _addressLabel = [XXLabel labelWithFrame:CGRectMake(kScreen_Width/2 - width/2, 0, width, self.height) text:KUser.rootAccount[@"BHAddress"] font:kFont(13) textColor:kDark100 alignment:NSTextAlignmentCenter];
    }
