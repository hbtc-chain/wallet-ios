...
@interface XXMessageSegmentView ()
...
@implementation XXMessageSegmentView
...
- (void)setItemsArray:(NSMutableArray *)itemsArray {
    ...
    for (NSInteger i=0; i < _itemsArray.count; i ++) {
        //button 实例
        XXMessageSegmentButton *itemButton = [XXMessageSegmentButton buttonWithType:UIButtonTypeCustom];
        [itemButton setLeftLabelText:_itemsArray[i]];
...
- (void)setUnreadNum:(NSNumber *)unreadNum buttonIndex:(NSInteger)index {
    XXMessageSegmentButton *itemButton = self.buttonArray[index];
    if (unreadNum && unreadNum.intValue > 0) {
        itemButton.rightLabel.hidden = NO;
        [itemButton setRightLabelText:[NSString stringWithFormat:@"%d ",unreadNum.intValue]];
    } else {
        itemButton.rightLabel.hidden = YES;
    }
}
