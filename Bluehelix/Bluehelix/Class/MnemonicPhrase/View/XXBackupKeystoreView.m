...
@interface XXBackupKeystoreView ()
...
@implementation XXBackupKeystoreView
...
- (UIView *)textBackView {
    if (!_textBackView) {
        _textBackView = [[UIView alloc] initWithFrame:CGRectMake(K375(24), CGRectGetMaxY(self.contentLabel3.frame) + 24, kScreen_Width - K375(48), 152)];
...
- (XXButton *)copyBtn {
    if (!_copyBtn) {
        MJWeakSelf
        _copyBtn = [XXButton buttonWithFrame:CGRectMake(K375(16), self.height - 64, kScreen_Width - K375(32), kBtnHeight) title:LocalizedString(@"CopyKeystore") font:kFontBold18 titleColor:[UIColor whiteColor] block:^(UIButton *button) {
            [weakSelf copyAction];
        }];
