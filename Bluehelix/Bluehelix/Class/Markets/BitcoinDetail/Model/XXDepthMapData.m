...
- (void)setVauesWithBids:(NSArray *)binds asks:(NSArray *)asks {
    ...
    if (success) {
        ...
        self.centerPrice = [dataDict[@"centerPrice"] doubleValue];
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (KDetail.blockList) {
                KDetail.blockList(dataDict[@"orderModels"], [dataDict[@"ordersAverage"] doubleValue], [dataDict[@"maxAmount"] doubleValue]);
            }
       ...
       dispatch_async(dispatch_get_main_queue(), ^{
           [KUser saveValeu:[self.dataArray mj_JSONString] forKey:@"ErrorDepthDataKey"];
           [self dismiss];
           [self show];
        });
    }
