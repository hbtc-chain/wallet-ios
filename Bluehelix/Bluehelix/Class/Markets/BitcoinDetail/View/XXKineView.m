...
/** 总数组 */
@property (strong, nonatomic, nullable) NSMutableArray *sumArray;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _serialQueue = dispatch_queue_create("com.kline.symbol", DISPATCH_QUEUE_SERIAL);
        ...
        MJWeakSelf
        KDetail.blockLoadNext = ^{
            [weakSelf loadDataOfNextPage];
        };
    }
    return self;
}
...
//    if (KConfigure.shareModel.watermarkImage) {
//        self.logoImageView.image = KConfigure.shareModel.watermarkImage;
//        self.logoImageView.width = KConfigure.shareModel.watermarkImage.size.width * self.logoImageView.height / KConfigure.shareModel.watermarkImage.size.height;
//    }
//
//    MJWeakSelf
//    KConfigure.shareModel.watermarkBlock = ^{
//        weakSelf.logoImageView.image = KConfigure.shareModel.watermarkImage;
//        weakSelf.logoImageView.width = KConfigure.shareModel.watermarkImage.size.width * weakSelf.logoImageView.height / KConfigure.shareModel.watermarkImage.size.height;
//    };
}
...
- (void)webSocketDataOfKline {
    ...
    self.hud.labelText= LocalizedString(@"Loading");
    ...
    self.webModel.params[@"params"] = @{@"limit":@(KlinCount),@"binary":@(YES)};
    ...
    self.webModel.httpParams[@"limit"] = @(KlinCount);
    
    [KQuoteSocket sendWebSocketSubscribeWithWebSocketModel:self.webModel];
}

#pragma mark - 5. 加载下一页数据
- (void)loadDataOfNextPage {
    
    KDetail.isLoading = YES;
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    params[@"symbol"] = [NSString stringWithFormat:@"%@.%@", KDetail.symbolModel.exchangeId, KDetail.symbolModel.symbolId];
    params[@"interval"] = self.type;
    params[@"limit"] = @(KlinCount);
    NSDictionary *dict = [self.sumArray firstObject];
    if ([dict[@"t"] longLongValue] > 0) {
        params[@"to"] = dict[@"t"];
    }
    MJWeakSelf
    [MBProgressHUD hideHUDForView:self animated:YES];
    ...
    [HttpManager quote_GetWithPath:@"quote/v1/klines" params:params andBlock:^(id data, NSString *msg, NSInteger code) {
        if (code == 0) {
            NSArray *dataArray = data[@"data"];
            if (dataArray.count < KlinCount) {
                KDetail.isNext = NO;
            } else {
                KDetail.isNext = YES;
            }
            [weakSelf receiveInsertData:dataArray];
        } else {
            KDetail.isLoading = NO;
            KDetail.isNext = YES;
            [MBProgressHUD hideHUDForView:weakSelf animated:YES];
...
- (void)receiveInsertData:(NSArray *)data {
    if (data.count > 1) {
        dispatch_async(_serialQueue, ^{
            [self receiveNextPageKlineData:data];
        });
    } else {
        KDetail.isLoading = NO;
        [MBProgressHUD hideHUDForView:self animated:YES];
...
- (void)receiveKlineData:(NSMutableArray *)data {
    ...
    if (self.webModel.isRed) {
        ...
        if (data.count < KlinCount) {
            KDetail.isNext = NO;
        } else {
            KDetail.isNext = YES;
        }
        
        if (self.isFirstReq) { // 切换币对或第一次收到数据
            self.isFirstReq = NO;
            dispatch_async(_serialQueue, ^{
                [self receiveFirstAllKlineData:data];
            });
        } else {
            dispatch_async(_serialQueue, ^{
                [self receiveReAllKlineData:data];
            });
        }
    } else {
        dispatch_async(_serialQueue, ^{
...
#pragma mark - 6.1 订阅【首次】收到全量k线数据
- (void)receiveFirstAllKlineData:(NSMutableArray *)data {

    self.sumArray = [NSMutableArray arrayWithArray:data];
    self.groupModel = [Y_KLineGroupModel objectWithArray:self.sumArray kLineType:self.type];
...
#pragma mark - 6.2 订阅后【降级或socket断开重连后】收到全量k线数据
- (void)receiveReAllKlineData:(NSMutableArray *)data{
    BOOL isAddkLine = NO;
    ...
    if (isAddkLine) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.stockChartView updateLineData:self.groupModel.models];
        });
    }
}

#pragma mark - 6.3 收到下一页数据
- (void)receiveNextPageKlineData:(NSArray *)data {
    for (NSInteger i=data.count - 2; i >= 0; i--) {
        NSDictionary *dict = data[i];
        [self.sumArray insertObject:dict atIndex:0];
    }
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        KDetail.isLoading = NO;
        [MBProgressHUD hideHUDForView:self animated:YES];
        [self.stockChartView insertLineData:self.groupModel.models insertCount:data.count - 1];
    });
}

#pragma mark - 6.4 收到【新增或更新】k线数据
