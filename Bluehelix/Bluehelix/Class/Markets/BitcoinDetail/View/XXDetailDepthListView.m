...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        MJWeakSelf
        KDetail.blockList = ^(NSMutableArray *modelsArray, double ordersAverage, double maxValue) {
            weakSelf.modelsArray = modelsArray;
            weakSelf.maxAmount = maxValue;
    ...
    [UIView animateWithDuration:0.2 animations:^{
        for (NSInteger i=0; i < self.cellsArray.count; i ++) {
            ...
            if (KDetail.symbolModel.type == SymbolTypeCoin) {
                cell.amounntIndex = KTrade.coinDepthAmount;
            }
            [cell reloadData];
        }
    }];
...
- (void)reloadLabelsValue {
    ...
    NSString *buyStrinng = [NSString stringWithFormat:@"%@(%@)", LocalizedString(indexType == 0 ? @"Amount" : @"Cumulative"), (KDetail.symbolModel.type == SymbolTypeCoin  || KDetail.symbolModel.type == SymbolTypeCoinMargin) ? KDetail.symbolModel.baseTokenName : LocalizedString(@"Paper")];
    ...
    NSString *sellString = [NSString stringWithFormat:@"%@(%@)", LocalizedString(indexType == 0 ? @"Amount" : @"Cumulative"), (KDetail.symbolModel.type == SymbolTypeCoin || KDetail.symbolModel.type == SymbolTypeCoinMargin) ? KDetail.symbolModel.baseTokenName : LocalizedString(@"Paper")];
...
- (void)amountButtonClick {
    ...
    MJWeakSelf
//    [XYHPickerView showPickerViewWithNamesArray:@[LocalizedString(@"Amount"), LocalizedString(@"Cumulative")] selectIndex:indexType Block:^(NSString *title, NSInteger index) {
//
//        if (index != indexType) {
//            if (KDetail.symbolModel.type == SymbolTypeCoin) {
//                KTrade.coinDepthAmount = index;
//            } else if (KDetail.symbolModel.type == SymbolTypeOption) {
//                KTrade.optionDepthAmount = index;
//            } else if (KDetail.symbolModel.type == SymbolTypeContract) {
//                KTrade.contractDepthAmount = index;
//            } else if (KDetail.symbolModel.type == SymbolTypeCoinMargin) {
//                KTrade.coinLeverDepthAmount = index;
//            }
//
...
- (XXButton *)buyButton {
    if (_buyButton == nil) {
        MJWeakSelf
        _buyButton = [XXButton buttonWithFrame:CGRectMake(KSpacing, 8, (kScreen_Width - KSpacing*2)/3.0, 32) title:[NSString stringWithFormat:@"%@(%@)", LocalizedString(@"Amount"), (KDetail.symbolModel.type == SymbolTypeCoin || KDetail.symbolModel.type == SymbolTypeCoinMargin) ? KDetail.symbolModel.baseTokenName : LocalizedString(@"Paper")] font:kFont12 titleColor:[UIColor assistTextColor] block:^(UIButton *button) {
...
- (XXButton *)sellButton {
    if (_sellButton == nil) {
        MJWeakSelf
        _sellButton = [XXButton buttonWithFrame:CGRectMake(CGRectGetMaxX(self.priceLabel.frame), self.buyButton.top, self.buyButton.width, self.buyButton.height) title:[NSString stringWithFormat:@"%@(%@)", LocalizedString(@"Amount"), (KDetail.symbolModel.type == SymbolTypeCoin || KDetail.symbolModel.type == SymbolTypeCoinMargin) ? KDetail.symbolModel.baseTokenName : LocalizedString(@"Paper")] font:kFont12 titleColor:[UIColor assistTextColor] block:^(UIButton *button) {
            [weakSelf amountButtonClick];
        }];
