...
- (void)reloadData:(NSArray *)data {
    ...
    NSMutableArray *dataArray = [NSMutableArray arrayWithArray:self.modelsArray];
    dispatch_async(dispatch_get_main_queue(), ^{
        [self reloadCellData:dataArray];
    });
}

- (void)reloadCellData:(NSMutableArray *)dataArray {
    for (NSInteger i=0; i < self.cellsArray.count; i ++) {
        XXTradeCell *cell = self.cellsArray[i];
        if (i < dataArray.count) {
            cell.hidden = NO;
            cell.model = dataArray[i];
        } else {
            cell.hidden = YES;
        }
    }
...
- (void)show {
    ...
    self.amountLabel.text = [NSString stringWithFormat:@"%@(%@)", LocalizedString(@"Amount"), (KDetail.symbolModel.type == SymbolTypeCoin || KDetail.symbolModel.type == SymbolTypeCoinMargin) ? KDetail.symbolModel.baseTokenName : LocalizedString(@"Paper")];
...
- (XXLabel *)amountLabel {
    if (_amountLabel == nil) {
        _amountLabel = [XXLabel labelWithFrame:CGRectMake(CGRectGetMaxX(self.priceLabel.frame), 16, self.dateLabel.width, 16) text:[NSString stringWithFormat:@"%@(%@)", LocalizedString(@"Amount"), (KDetail.symbolModel.type == SymbolTypeCoin || KDetail.symbolModel.type == SymbolTypeCoinMargin) ? KDetail.symbolModel.baseTokenName : LocalizedString(@"Paper")] font:kFont12 textColor:[UIColor assistTextColor]];
