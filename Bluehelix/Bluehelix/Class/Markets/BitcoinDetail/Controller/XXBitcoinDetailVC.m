...
- (void)showAndReloadSymbolData {
    ...
    // 7. 收藏按钮
    self.saveButton.selected = KDetail.symbolModel.favorite;
}
...
- (void)switchButtonClick:(UIButton *)sender {
    ...
    // 统一修改为夜间模式
    BOOL isNight = KUser.isNightType;
    if (!isNight) {
        KUser.isNightType = YES;
    }
    
    // 判断类型
    if (KDetail.symbolModel.type == SymbolTypeCoin) { // 币币
        XXSwitchCoinView *switchPairsView = [[XXSwitchCoinView alloc] initWithFrame:CGRectMake(0, 0, kScreen_Width, kScreen_Height)];
        switchPairsView.isSymbolDetail = YES;
        MJWeakSelf
        switchPairsView.detailSymbolBlock = ^(NSInteger index) {
            if (index == 1) { // 清理数据
                [weakSelf cleanData];
            } else { // 重新订阅
                [weakSelf showAndReloadSymbolData];
                KUser.isNightType = isNight;
            }
        };
