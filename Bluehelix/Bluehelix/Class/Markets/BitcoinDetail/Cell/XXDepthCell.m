...
- (void)reloadData {
    ...
    if (self.model.leftPrice) {
        ...
        double molecular = self.model.leftSumNumber; // 分子
        double denominator = self.ordersAverage; // 分母
        if (self.amounntIndex == 0) {
            molecular = [self.model.leftNumber doubleValue];
            denominator = self.maxAmount;
        ...
        CGFloat leftValue = molecular/denominator;
        if (isnan(leftValue) || isinf(leftValue)) {
            leftValue = 1.0;
        }
        if (leftValue >= 1.0) {
    ...
    if (self.model.rightPrice) {
        ...
        double molecular = self.model.rightSumNumber; // 分子
        double denominator = self.ordersAverage; // 分母
        if (self.amounntIndex == 0) {
            molecular = [self.model.rightNumber doubleValue];
            denominator = self.maxAmount;
        ...
        CGFloat rightValue = molecular/denominator;
        if (isnan(rightValue) || isinf(rightValue)) {
            rightValue = 1.0;
        }
        if (rightValue >= 1.0) {
